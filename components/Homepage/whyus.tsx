...
export default function WhyChooseUs() {
  return (
    <section className="relative px-4 pb-12 sm:px-6 lg:px-8 mx-auto text-center bg-gray-100 dark:bg-gray-700 dark:text-white">
        ...
        {items.map((item, i) => (
          <div key={i} className="flex flex-col items-center">
            <div className="rounded-t-full w-full border-2 border-b-0 p-3 border-dashed border-black dark:border-white overflow-hidden">
              <div className={`pt-6 flex flex-col pb-32 justify-center items-center relative aspect-[1/1] pb-2 px-4 rounded-t-full inset-shadow-xs inset-shadow-white`} style={{backgroundColor:item.color}}>
            ...
            <p className="text font-bold text-gray-700 dark:text-white mt-4 max-w-xs">{item.text}</p>
          </div>
        ))}
      </div>
    </section>
